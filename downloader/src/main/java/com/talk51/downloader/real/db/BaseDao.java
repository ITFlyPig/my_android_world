...
public abstract class BaseDao<T> {
    ...
    public BaseDao(SQLiteOpenHelper helper) {
        ...
        //每次新建实例的时候就检查一下对应的表是否创建，因为表是动态创建的
        if (helper instanceof DBHelper) {
            DBHelper dbHelper = (DBHelper) helper;
            this.database.execSQL(dbHelper.getDownloadTableEntity().buildTableString());
        }
    }
    ...
    public boolean insert(T t) {
        ...
        try {
            ...
            long rowId = database.insert(getTableName(), null, getContentValues(t));
